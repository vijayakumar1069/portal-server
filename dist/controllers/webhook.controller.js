...
export const handleFreshdeskWebhook = async (req, res) => {
    try {
        const { freshdesk_webhook: webhook, ticket_id, ticket_url, ticket_subject, ticket_description, ticket_status, ticket_priority, ticket_created_at, ticket_updated_at, requester_email, requester_name, requester_phone, agent_email, agent_name, group_name, product_name, custom_fields } = req.body;
...
const handleTicketCreated = async (ticketData, userId) => {
...
const handleTicketUpdated = async (ticketData, userId) => {
...
const handleTicketResolved = async (ticketData, userId) => {
...
const handleTicketClosed = async (ticketData, userId) => {
...
export const getWebhookLogs = async (req, res) => {
    try {
        ...
        const { page = 1, limit = 20, event, source, startDate, endDate } = req.query;
        // Build filter
        const filter = { userId };
        if (event) {
            filter.event = event;
